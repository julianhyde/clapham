...
public class Java2DChart implements Chart {
	...
	public void drawComponent(Symbol s) {
		...
		symbolSize = new Size(s.graph.graphSize.getWidth()
				+ beginningXCoordinate + componentGapWidth * 2,
				s.graph.graphSize.getHeight() + beginningYCoordinate
						+ componentGapHeight * 2 + 5);
		// EbnfForm.Drawarea=new
		// Bitmap(Node.getSymbolSize().getWidth(),Node.getSymbolSize().getHeight(),
		...
		drawString(s.name, beginningXCoordinate - 20, beginningYCoordinate - 30);
		// g.DrawRectangle(new
		// Pen(Color.Orange,2),p.X,p.Y+30,s.graph.graphSize.getWidth(),s.graph.graphSize.getHeight());
