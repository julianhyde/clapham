...
public class Grammar {
	...
	boolean deepCompare(Node n1, Node n2, boolean untilIter) {
		...
		while (n1 != null && samelevel) {
			...
			if (untilIter) {
				if (n1.typ == NodeType.ITER && n1.sub == identifier) {
					if (n1 == n2) { // last iter node's next points to the iter
						if (TRACE) {
							System.out
									.println("true: iter node reached, graphs match");
						}
						return true;
					} else {
						if (TRACE) {
							System.out
									.println("false: iter node reached, graphs do not match");
						}
						return false;
					}
				}
			...
			if (n2 == null) {
				if (TRACE) {
					System.out
							.println("false: second enclosing substructure ended before first");
				}
				return false;
			}
			if (!compare(n1, n2)) {
			...
			if (n1.typ == NodeType.OPT || n1.typ == NodeType.ITER
					|| n1.typ == NodeType.RERUN) {
				if (!deepCompare(n1.sub, n2.sub, false)) {
					if (TRACE) {
						System.out
								.println("false: false in subelem of iter,opt or rerun");
					}
					return false;
				}
				if (n1.typ == NodeType.RERUN
					...
					if (TRACE) {
						System.out
								.println("false: itergraph of rerun doesn't match");
					}
					return false;
				}
				...
				while (a1 != null) {
					if (a2 == null) {
						if (TRACE) {
							System.out
									.println("false: false in subalt, second node null");
						}
						return false;
					}
			...
			if (n1.up) {
				if (!n2.up) {
					if (TRACE) {
						System.out
								.println("false: second has not finished enclosing structure");
					}
					return false;
				}
				samelevel = false;
		...
		if (n1 == null && n2 != null) {
			if (TRACE) {
				System.out
						.println("false: first enclosing substructure ended before second");
			}
			return false;
		}
		return true;
	...
	void removeWrongLinebreaks(Node n, Node parent, Symbol s) {
		...
		while (i != null && samelevel) {
			...
			} else if (i.typ == NodeType.OPT || i.typ == NodeType.ITER
	...
	void removeRedundancy(Node n, Node parent, Symbol s) {
		...
		while (n != null && samelevel) {
			if (n.typ == NodeType.ALT) {
				...
				while ((n.sub.typ == NodeType.ITER || n.sub.typ == NodeType.OPT)
						&& n.sub.up) {
				...
				while ((n.sub.typ == NodeType.OPT && (n.sub.up || n.sub.next == null))
						|| (n.sub.typ == NodeType.ITER && (n.sub.up || n.sub.next == null))) {
					// if(n.sub.typ==Node.opt || containsIter)
					// EbnfForm.WriteLine("Rendundant "+Node.nTyp[n.sub.typ]+" Node removed (opt).");
		...
		while (n != null && samelevel) {
			...
			if (n.typ == NodeType.EPS) {
				...
				while (a != null) {
					...
					if (!(a.sub.typ == NodeType.EPS && (a.sub.next == null || a.sub.up))) {
						removeEps(a.sub, a, s);
					}
					a = a.down;
				}
		...
		if (containsEps) {
			...
			while (a != null) {
				...
				if (a.sub.typ == NodeType.EPS
					...
					break; // there can be only one eps in the alts because same
			...
			if (!(alt.sub.typ == NodeType.EPS && (alt.sub.next == null || alt.sub.up))) {
				...
				while (j != i && !matchFound) {
					...
					while (k != null && samelevel2 && !matchFound) {
						if (deepCompare(j, k, true)) {
							...
							matchFound = true;
							// replace the iter node and the nodes before by the
							// rerun node
	...
	public void printNodes(PrintStream out) {
		...
		for (Node p : nodes) {
			...
			case OPT:
			case RERUN:
				out.format("%1$5d %2$5d", Grammar.ptr(p.down, false), Grammar
						.ptr(p.sub, false));
				break;
			case EPS:
				out.print("           ");
